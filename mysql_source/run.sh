...
if [ -e ${pv} -a -e ${pvc} -a -e ${rc} ]; then
  ...
  if [ ${#var[0]} == 0 ]; then
    echo There is no Service like \"${param_svc}\", need to create new one;
    kubectl create -f ${svc}
    echo A new Service created;
  else
    echo There is already a Service , no need to create a new one;
    echo ${var[0]}
  fi
  unset var
else
...
unset pv pvc rc svc param_pv param_pvc param_rc param_svc
